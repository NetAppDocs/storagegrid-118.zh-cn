...
如果无法访问网格管理器、则可以在包含ADC服务的某些存储节点上找到恢复软件包的加密副本。在每个存储节点上、检查恢复软件包的此位置： `/var/local/install/sgws-recovery-package-_grid-id_-_revision_.zip.gpg` 使用修订版号最高的恢复软件包。
...
有关您的平台当前支持的版本，请参见 https://imt.netapp.com/matrix/#welcome["NetApp 互操作性表工具"^]。
...
下载软件并提取文件，除非您这样做 link:recovering-from-storage-node-failures.html["恢复存储节点上的故障存储卷"]。
...
. 如果要恢复 link:../rhel/index.html["Red Hat Enterprise Linux系统"]下，选择相应的文件。
...
|===
. 如果要恢复 link:../ubuntu/index.html["Ubuntu 或 Debian 系统"]下，选择相应的文件。
...
|===
. 如果要恢复 link:../vmware/index.html["VMware 系统"]下，选择相应的文件。
