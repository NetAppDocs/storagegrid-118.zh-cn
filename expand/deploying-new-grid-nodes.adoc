...
.步骤
. link:../vmware/deploying-storagegrid-node-as-virtual-machine.html["将新节点部署为虚拟机"] 并将其连接到一个或多个 StorageGRID 网络。
+
部署节点时，您可以选择重新映射节点端口或增加 CPU 或内存设置。

. 部署所有新的 VMware 节点后， link:performing-expansion.html["执行扩展操作步骤"]。
...
. 如果要向现有Linux主机添加节点、请使用StorageGRID 主机服务命令行界面启动新节点：``sudo storagegrid node start [<node name\>]``


.完成后
部署所有新网格节点后，您可以 link:performing-expansion.html["执行扩展"]。
...
* 您的服务笔记本电脑具有 link:../admin/web-browser-requirements.html["支持的 Web 浏览器"]。
